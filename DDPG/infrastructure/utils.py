...
def optimize_model(memory, critic_net, target_critic_net, actor_net, target_actor_net, actor_optimizer, critic_optimizer, device):
    ...
    non_final_next_states = torch.cat([s for s in batch.next_state if s is not None]).to(device, dtype=torch.float32)
    state_batch = torch.cat(batch.state).to(device, dtype=torch.float32)
    action_batch = torch.cat([torch.tensor(a, device=device, dtype=torch.float32) for a in batch.action]).to(device, dtype=torch.float32)
    ...
    next_state_values[non_final_mask] = target_critic_net(non_final_next_states, next_actions).squeeze()
